...
async def list_proxy_hosts(_user: AdminUser, db: SessionDep) -> list[ProxyHostRead]:
...
async def get_proxy_host(host_id: int, _user: AdminUser, db: SessionDep) -> ProxyHostRead:
